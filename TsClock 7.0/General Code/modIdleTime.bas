...
'call the StartTracking procedure. To cancel the
'tracking, call the StopTracking procedure.
'
'NB, Please note the following;
'1) If StartTracking has been called within your
'   program, StopTracking MUST also be called
'   within your program.
'
...
'   StopTracking in the QueryUnload event on one
...
Private midlSystem          As clsIdleDll   'holds the system-wide idle time
...
Public Sub NotIdle(ByVal enmFrom As EnumCalledFrom, _
    ...
    '----- Timed Shutdown Clock 7.0 -- 29/10/2002 -----
    
    'update the idle time
    If gblnIdleShut Then
        Call frmClock.UpdateClockAlarm
    End If
    
    'check if the form is being dragged
    If (enmFrom = FROM_MOUSE) Then
        'the mouse is being moved over the form
        Call frmClock.CheckDrag
    End If
End Sub
...
'    Exit Sub
    Call StopTracking
    If Not midlSystem Is Nothing Then
        'reset
        midlSystem.Start
    Else
        'start new
        Set midlSystem = New clsIdleDll
    End If
...
Public Function CurrentIdleTime() As Long
    ...
    If mblnTracking Then
        If Not midlSystem Is Nothing Then
            CurrentIdleTime = midlSystem.IdleTime
        End If
    End If
End Function
...
Private Function GetMouseButtonState() As MouseButtonConstants
    ...
    Const KEY_DOWN          As Long = -32767
    Const KEY_PRESSED       As Long = -32768
    Const KEY_NOT_PRESSED   As Long = 0
